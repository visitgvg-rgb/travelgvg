...
const EventDetailPage: React.FC = () => {
    ...
    useEffect(() => {
        const fetchEvents = async () => {
            ...
            try {
                ...
                const foundItem = data.find(e => e.id === slug);
    ...
    if (!item) {
        return (
            <div className="text-center py-16">
                <p className="text-gray-500 dark:text-gray-400">{t('eventPage.noResults')}</p>
            </div>
        );
    }
    ...
    const renderConnectSection = () => {
        const showFacebook = !!itemContact?.facebook;
        const showInstagram = !!itemContact?.instagram;
        const hasWebsite = itemContact?.website;
            ...
            <div className="flex flex-wrap items-center gap-4 mb-6">
                ...
                {hasWebsite && (
                    <a
                        ...
                        className='bg-gray-800 text-white hover:bg-gray-900 font-bold py-2 px-4 rounded-full transition-colors duration-300 text-sm shadow-md flex items-center gap-2'
                    >
                        ...
                        <p className="text-gray-600 dark:text-gray-300 mb-6 whitespace-pre-line">{item.description[lang]}</p>
                         {renderConnectSection()}
                          <div className="flex flex-col gap-3 w-full">
                                {item.contact.phone && (
                                    <a href={`tel:${item.contact.phone}`} className='w-full flex items-center justify-center bg-gray-800 text-white hover:bg-gray-900 font-black py-3.5 px-6 rounded-xl shadow-lg hover:shadow-xl hover:scale-[1.02] transition-all duration-200'>
